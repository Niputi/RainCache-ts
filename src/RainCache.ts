...
interface TCaches {role?: RoleCache, emoji?: EmojiCache, permOverwrite?: PermissionsOverwriteCache, user?: UserCache, member?: MemberCache, presence?: PresenceCache, channelMap?: ChannelMapCache, channel?: ChannelCache, guild?: GuildCache }
...
class RainCache extends EventEmitter {
     ...
     */
    constructor(options : {storage : BaseStorageEngine, disabledEvents: any, cacheClasses?: Object}, inboundConnector: BaseStorageEngine, outboundConnector: BaseStorageEngine) {
        super();
        if (!options.storage) {
    ...
    async initialize() {
        ...
        if (this.inbound) {
            this.inbound.on('event', async (event: any) => {
                try {
    ...
    _createCaches(engines : BaseCache[], cacheClasses: any) {
        let caches : TCaches = {};
    ...
    _getEngine(engines : BaseCache[], engine: any) {
