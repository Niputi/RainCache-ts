...
class GuildCache extends BaseCache {
     ...
     */
    async get(id: string): Promise<Object | null | GuildCache> {
        if (this.boundObject) {
     ...
     */
    async update(id: string, data: {channels?: TChannel[], members?: TGuildMember[], presences?: any[], emojis?: TEmoji[], roles?: TRole[]}): Promise<this | GuildCache> {
        if (this.boundObject) {
     ...
     */
    async filter(fn: Function) : Promise<GuildCache[]> {
        // possible bug here
        let guilds: TGuild[] = await this.storageEngine.filter(fn, null, this.namespace);
     ...
     * @returns {Promise.<GuildCache>} - returns a bound guild cache
     */
    async find(fn:  Function) : Promise<GuildCache> {
        // possible bug here
        let guild: TGuild = await this.storageEngine.find(fn, null, this.namespace);
        return new GuildCache(this.storageEngine, this.channels.bindGuild(guild.id), this.roles.bindGuild(guild.id), this.members.bindGuild(guild.id), this.emojis.bindGuild(guild.id), this.presences.bindGuild(guild.id), this.guildChannelMap.bindGuild(guild.id), guild);
    }
