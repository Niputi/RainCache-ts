...
import { TGuildMember } from "../LibTypes";
...
class MemberCache extends BaseCache {
     ...
     */
    async get(id : string, guildId = this.boundGuild): Promise<object| MemberCache | null> {
        if (this.boundObject) {
     ...
     */
    async update(id, guildId = this.boundGuild, data): Promise<MemberCache> {
        if (this.boundObject) {
     ...
     */
    async filter(fn: Function, guildId = this.boundGuild, ids : string[] | null = null): Promise<Array<MemberCache>> {
        let members: TGuildMember[] = await this.storageEngine.filter(fn, ids, super.buildId(guildId));
     ...
     */
    async find(fn: Function, guildId = this.boundGuild, ids: string[] | null = null): Promise<MemberCache> {
        let member: TGuildMember = await this.storageEngine.find(fn, ids, super.buildId(guildId));
        return new MemberCache(this.storageEngine, this.user.bindUserId(member.id), member);
    }
     ...
     * @return {*}
     */
    buildId(userId: string, guildId: string) {
        if (!guildId) {
