...
class PermissionOverwriteCache extends BaseCache {
     ...
     */
    async get(id: string, channelId: string = this.boundChannel) : Promise<PermissionOverwriteCache | null | Object> {
        if (this.boundObject) {
     ...
     */
    async update(id: string, channelId: string = this.boundChannel, data: TPermissionOverwrite) : Promise<PermissionOverwriteCache> {
        if (this.boundObject) {
     ...
     * @return {Promise.<void>}
     */
    async remove(id : string, channelId: string = this.boundChannel) : Promise<void> {
        if (this.boundObject) {
     ...
     */
    async filter(fn :Function, channelId : string = this.boundChannel, ids: string[] | null = null) : Promise<PermissionOverwriteCache[]> {
     ...
     * @return {Promise.<PermissionOverwriteCache>} - returns a bound permission overwrite cache
     */
    async find(fn : Function, channelId: string = this.boundChannel, ids : string[] | null = null) : Promise<PermissionOverwriteCache> {
        let permissionOverwrite: TPermissionOverwrite = await this.storageEngine.find(fn, ids, super.buildId(channelId));
        return new PermissionOverwriteCache(this.storageEngine, permissionOverwrite);
    }
