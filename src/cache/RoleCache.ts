...
class RoleCache extends BaseCache {
     ...
     */
    async get(id: string, guildId : string): Promise<RoleCache|  null | Object> {
        if (this.boundObject) {
     ...
     */
    async update(id: string, guildId: string, data: TRole) : Promise<RoleCache> {
        if (this.boundObject) {
     ...
     * @return {Promise.<void>}
     */
    async remove(id: string, guildId: string) : Promise<void> {
        if (this.boundObject) {
     ...
     */
    async filter(fn: Function, guildId = this.boundGuild, ids: string[] | null = null) : Promise<RoleCache[]> {
        let roles: TRole[] = await this.storageEngine.filter(fn, ids, super.buildId(guildId));
     ...
     */
    async find(fn: Function, guildId = this.boundGuild, ids: string[] | null = null): Promise<RoleCache> {
        let role: TRole = await this.storageEngine.find(fn, ids, super.buildId(guildId));
        return new RoleCache(this.storageEngine, role);
    }
