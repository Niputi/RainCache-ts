...
class UserCache extends BaseCache {
     ...
     */
    async get(id = this.id) : Promise<UserCache | null| Object> {
        if (this.boundObject) {
     ...
     */
    async update(id: string = this.id, data: TUser) : Promise<UserCache> {
        if (this.boundObject) {
     ...
     * @return {Promise.<void>}
     */
    async remove(id: string = this.id): Promise<void> {
        if (this.boundObject) {
     ...
     * @return {Promise.<UserCache[]>}
     */
    async filter(fn : Function, ids : string[] | null = null):  Promise<UserCache[]> {
        let users : TUser[] = await this.storageEngine.filter(fn, ids, this.namespace);
     ...
     * @return {Promise.<UserCache|null>} - Returns a User Cache with a bound user or null if no user was found
     */
    async find(fn : Function, ids : string[] | null = null) : Promise<UserCache | null> {
        let user: TUser = await this.storageEngine.find(fn, ids, this.namespace);
        if (!user) {
            return null;
        }
        return new UserCache(this.storageEngine, user);
     ...
     * @return {Promise.<void>}
     */
    async removeIndex() : Promise<void> {
